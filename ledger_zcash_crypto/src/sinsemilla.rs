...
/// The largest integer such that `2^C <= (r_P - 1) / 2`.
pub const C: usize = 253;

const ORCHARD_NOTE_COMMITMENT_PERSONALIZATION: &str = "z.cash:Orchard-NoteCommit";
const ORCHARD_COMMIT_IVK_PERSONALIZATION: &str = "z.cash:Orchard-CommitIvk";

type AffineCoordinates = ([u8; 32], [u8; 32]);

const Q_NOTE_COMMITMENT_M_GENERATOR: AffineCoordinates = (
    [
    ...
    [
        0x63, 0xac, 0x49, 0x73, 0x5a, 0x0a, 0x27, 0x87, 0x9e, 0x5e, 0xdb, 0x81, 0x88, 0x12, 0x22,
        0x88, 0x2c, 0xc9, 0xf4, 0x6e, 0xd9, 0xc2, 0xbe, 0x4e, 0x83, 0x70, 0xc6, 0x8a, 0x93, 0x58,
        0xa0, 0x32,
    ],
);

const Q_COMMIT_IVK_M_GENERATOR: AffineCoordinates = (
    [
    ...
    [
        0xbe, 0xde, 0xad, 0xcf, 0xce, 0xe5, 0x5a, 0xbe, 0xf1, 0xa5, 0x6d, 0xc9, 0x1d, 0x35, 0xc4,
        0x46, 0x4b, 0x05, 0xde, 0x20, 0x46, 0x07, 0x59, 0xef, 0xe6, 0xbe, 0x1a, 0xd4, 0xf6, 0x4c,
        0x01, 0x1b,
    ],
);

const R_NOTE_COMMITMENT_GENERATOR: AffineCoordinates = (
    [
    ...
    [
        0xe3, 0x37, 0xcf, 0x22, 0xc6, 0x53, 0x41, 0xc3, 0xf6, 0xdc, 0xe0, 0x23, 0x78, 0xc0, 0xc7,
        0x79, 0xc6, 0xe1, 0x66, 0x23, 0x2f, 0x1a, 0x54, 0xa9, 0x4c, 0xc0, 0x6d, 0x68, 0x4b, 0x56,
        0xde, 0x3c,
    ],
);

const R_COMMIT_IVK_GENERATOR: AffineCoordinates = (
    [
    ...
    [
        0xa9, 0xdd, 0x7f, 0xe3, 0xb3, 0x93, 0xe7, 0x3f, 0xc7, 0xa6, 0x58, 0x1b, 0xfb, 0x42, 0x44,
        0x6b, 0x94, 0x57, 0x4b, 0x28, 0xc4, 0x90, 0xc8, 0xc2, 0xeb, 0xfa, 0xa2, 0x66, 0x99, 0xd2,
        0xcf, 0x29,
    ],
);
...
fn commit_domain_generators(domain: &str) -> Result<(AffineCoordinates, AffineCoordinates), Error> {
    match domain {
        ORCHARD_NOTE_COMMITMENT_PERSONALIZATION => {
            Ok((Q_NOTE_COMMITMENT_M_GENERATOR, R_NOTE_COMMITMENT_GENERATOR))
        }
        ORCHARD_COMMIT_IVK_PERSONALIZATION => {
            Ok((Q_COMMIT_IVK_M_GENERATOR, R_COMMIT_IVK_GENERATOR))
        }
        _ => Err(Error::UnsupportedSinsemillaDomain),
    }
}
...
fn chunk_to_index(chunk: &[bool]) -> usize {
    ...
    lebs2ip_k(&bits) as usize
}

fn lebs2ip_k(bits: &[bool; K]) -> u32 {
    bits.iter()
        .enumerate()
...
fn point_from_affine_repr(x_le: &[u8; 32], y_le: &[u8; 32]) -> Result<EcPoint, Error> {
    let mut x_be = [0u8; 32];
    let mut y_be = [0u8; 32];
    reverse_copy(&mut x_be, x_le);
    reverse_copy(&mut y_be, y_le);
